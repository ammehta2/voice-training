"""
Feature extraction for the opening-phrase pipeline (2.5s window).
...
Output: data/opening/features/X_{train,val,test}.npy and labels.
"""
...
POS_DIR = Path("data/opening/positive_aug")
NEG_DIR = Path("data/opening/negative_aug")
OUT = Path("data/opening/features")
...
def source_of(filename: str) -> str:
    """Group all augmentations of a source together.

    real_navkar_03_opening_aug05_medium.wav  -> real_navkar_03_opening
    real_navkar_03_opening_orig.wav          -> real_navkar_03_opening
    midmantra_real_navkar_01_000_aug03_light.wav -> midmantra_real_navkar_01_000
...
def main() -> None:
    ...
    # Group mid-mantra negatives by parent recording so they stay together
    def reduce_to_parent(s: str) -> str:
        # midmantra_real_navkar_01_023 → real_navkar_01  (matches both _01 and _14a/_14b)
        m = re.match(r"^midmantra_(real_navkar_\d+[ab]?)", s)
        if m:
    ...
    print(f"  After mid-mantra grouping: {len(neg_groups)} negative source groups")
    ...
    # For each positive source, find its mid-mantra negative key and FORCE it
    # into the same split. This avoids speaker leakage where opening of recording
    # N is in train but mid-mantra of N is in test (or vice-versa).
    def opening_to_midmantra(s: str) -> str:
        # real_navkar_01_opening → midmantra_real_navkar_01
        m = re.match(r"^(real_navkar_\d+[ab]?)_opening", s)
        return f"midmantra_{m.group(1)}" if m else None

    tied_train = {opening_to_midmantra(s) for s in pos_tr_src} - {None}
    tied_val = {opening_to_midmantra(s) for s in pos_v_src} - {None}
    tied_test = {opening_to_midmantra(s) for s in pos_te_src} - {None}

    # Split AMBIENT negatives (everything that's NOT mid-mantra) randomly
    ambient_keys = [k for k in neg_groups if not k.startswith("midmantra_")]
    amb_tr, amb_v, amb_te = split_sources(ambient_keys, rng)

    # Combine: mid-mantra tied to positive splits + ambient random split
    neg_tr_src = list(tied_train | set(amb_tr))
    neg_v_src = list(tied_val | set(amb_v))
    neg_te_src = list(tied_test | set(amb_te))
    print(f"  Tied mid-mantra: train={len(tied_train)}, val={len(tied_val)}, test={len(tied_test)}")
