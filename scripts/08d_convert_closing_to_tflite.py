...
train_module = import_module("05d_train_closing")
ClosingDetector = train_module.ClosingDetector

INPUT_TIME_FRAMES = 251  # matches feature extraction output
INPUT_FEATURES = 40
...
def main() -> None:
    ...
    # Load model
    model = ClosingDetector(dropout=0.0)  # dropout off for export
    ...
    calib = None
    train_npy = Path("data/closing/features/X_train.npy")
    if train_npy.exists():
        X = np.load(train_npy)
